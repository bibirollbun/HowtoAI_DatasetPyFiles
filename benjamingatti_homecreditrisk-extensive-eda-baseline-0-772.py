...
bureau = pd.read_csv(path + "bureau.csv") #.set_index('SK_ID_CURR')
...
data = pd.concat([train_X, test_X], axis=0).set_index('SK_ID_CURR')
...
data = data.merge(right=bflattened.reset_index(), how='left', on='SK_ID_CURR')
gc.collect()
data.head()
...
#year must more significant leaving epoch out for now
...
lgb.plot_importance(model, figsize=(12, 35), max_num_features=150);
