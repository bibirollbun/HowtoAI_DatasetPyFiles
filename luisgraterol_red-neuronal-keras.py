...
arquitectura1 = [4096,2048,1024,1024]
...
arquitectura2 = [1024,1024,256,256]
model2 = func_model(arquitectura2)
#print(model2.summary())
train_history_tam2 = model2.fit(train_dfX, train_dfY, batch_size=512, epochs=20, validation_data=(val_dfX, val_dfY))
graf_model(train_history_tam2)
precision(model2)


arquitecturaFinal = [1024,1024,256,256]
modelF = func_model(arquitecturaFinal)
print(modelF.summary())
train_history_tamF = modelF.fit(train_dfX, train_dfY, batch_size=512, epochs=20, validation_data=(val_dfX, val_dfY))
...
def func_model_reg():   
    ...
    x=Dropout(0)(x)
    x=Dense(256, activation="relu", kernel_initializer='random_uniform', bias_initializer='zeros', kernel_regularizer=None)(x)
    x=Dropout(0)(x)
    x=Dense(256, activation="relu", kernel_initializer='random_uniform', bias_initializer='zeros', kernel_regularizer=None)(x)
    x=Dropout(0)(x)  
    x=Dense(1, activation="sigmoid", kernel_initializer='random_uniform', bias_initializer='zeros')(x) 
