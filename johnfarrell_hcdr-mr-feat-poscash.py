...
df['cnt_ins_curr'] = df['CNT_INSTALMENT'] - df['CNT_INSTALMENT_FUTURE']
...
%%time
cache_dirs = ['./', '../input/hcdr-mr-feat-poscash/']
feat_prev = None
main_key = 'SK_ID_PREV'
for d in cache_dirs:
...
# %%time
# main_key = 'cnt_ins_curr'
# feat = Parallel(n_jobs=8)(
#     delayed(func)(key, group, main_key, 'cnt_curr_') for key,group in tqdm(
#         df.groupby([main_key], sort=False)
#     )
# )
# feat = pd.DataFrame(feat)
...
%%time
main_key = 'CNT_INSTALMENT'
feat = Parallel(n_jobs=8)(
    delayed(func)(key, group, main_key, 'cnt_ins_') for key,group in tqdm(
        df.groupby([main_key], sort=False)
    )
)
feat = pd.DataFrame(feat)
