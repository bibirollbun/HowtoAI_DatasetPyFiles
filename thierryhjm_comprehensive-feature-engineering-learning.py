...
transaction_train = pd.read_csv('../input/ieee-fraud-detection/train_transaction.csv',index_col='TransactionID')
...
identity_test = pd.read_csv('../input/ieee-fraud-detection/test_identity.csv',index_col='TransactionID')
...
#################################################################################
# Converting Strings to ints(or floats if nan in column) using frequency encoding
# for id information in identity table
# encoding seperately
...
#export the feature to save time if we re-run
...
######################################################
#calculate transaction amount by group (mean, std transaction encoding)
...
for feature in ['id_02__id_20', 'id_02__D8', 'D11__DeviceInfo', #'DeviceInfo__P_emaildomain', 
                'P_emaildomain__C2', 'card2__dist1', 'card1__card5', 'card2__id_20', 'card5__P_emaildomain', 'addr1__card1']:
...
train_Y.to_csv('train_FE_Y.csv',index=False,header=False)





# read from saving files
train_X = pd.read_csv('./train_FE.csv', header=None)
train_Y = pd.read_csv('./train_FE_Y.csv', header=None)
test_X = pd.read_csv('./test_FE.csv', header=None)


for col in train_X.columns:
    if train_X[col].dtypes == 'object':
        train_X.drop([col], axis=1)
        if 'id_' in col:
            col = col.replace('_', '-')
            test_X.drop([col], axis=1)
...
#pred = pd.read_csv('../input/prediction-lgb/prediction_model_lgb_train_2.csv')
#pred.to_csv("submission.csv", index=False)
