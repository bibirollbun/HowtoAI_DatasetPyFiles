...
#sns.boxplot(app_train['DAYS_BIRTH'],app_train['TARGET'])


app_train[['DAYS_BIRTH', 'TARGET']].sort_values('DAYS_BIRTH')
...
submit.head()
submit.to_csv('_subm.csv', index=False)
...
print(log_reg.score(train,train_labels))
rmse_cv(log_reg).mean()
...
print(random_forest.score(train,train_labels))
#rmse_cv(random_forest).mean
