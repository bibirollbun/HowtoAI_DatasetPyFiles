...
def application_train():
    ...
    df['DIFF_PRICE_CREDIT']=(df['AMT_GOODS_PRICE']!=df['AMT_CREDIT']).apply(lambda x: 1 if x==True else 0)
...
def bureau_bb():
    ...
    #BUREAU BALANCE and BUREAU join
...
def previous_application(agg_list_previous_application):
    ...
    prev_agg_list = {"SK_ID_CURR":["count"], 
                ...
                "INTEREST_RATE":["median", "mean", "min", "max"],
...
def main():
    ...
    with timer("Modeling"):
        ...
        feat_importance.to_csv('feat_importaance.csv', index=False)
